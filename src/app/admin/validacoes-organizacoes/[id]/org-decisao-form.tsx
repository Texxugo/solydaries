...
import {
  approveOrgValidationAction,
  rejectOrgValidationAction,
...
export function OrgDecisaoForm({ requestId }: { requestId: string }) {
      ...
      {mode === "approve" ? (
        <form action={approveAction} className="flex flex-col gap-4">
          ...
          <p className="text-sm text-stone-600">
            A organização recebe o selo de validação, fica habilitada a ter
            página pública e a criar campanhas em nome próprio. Os
            representantes serão notificados.
          </p>
          <button
            type="submit"
            disabled={pending}
        ...
        <form action={rejectAction} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <textarea
              ...
              maxLength={1000}
              required
              placeholder="Ex.: documentação institucional incompleta..."
              className={inputClassName}
            />
            <p className="mt-1.5 text-xs text-stone-400">
              O motivo é enviado aos representantes, que poderão corrigir e
              reenviar.
            </p>
          </div>
          <button
