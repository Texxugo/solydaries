...
export const metadata: Metadata = {
  title: "Validações pendentes — Solydaries",
};
...
export default async function ValidacoesPage() {
  ...
  const pending = await prisma.personValidationRequest.findMany({
      ...
      {pending.length === 0 ? (
        ...
        <ul className="divide-y divide-stone-100 rounded-2xl border border-stone-100">
          {pending.map((request) => (
            <li key={request.id}>
              <Link
                href={`/admin/validacoes/${request.id}`}
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-stone-500">
                    {request.person.email} · {request.documents.length}{" "}
                    documento(s) · enviada em{" "}
